...
export class StarSystem {
  ...
  async getCurrentUserStars() {
    try {
      ...
      if (!ref) return 0;
      const [db, col, uid] = ref;
      const { getDoc, doc } = await import('https://www.gstatic.com/firebasejs/10.7.1/firebase-firestore.js');
      const snap = await getDoc(doc(db, col, uid));
      const data = snap.exists() ? snap.data() : {};
      return (typeof data.stars === 'number') ? data.stars : 0;
    } catch (e) {
      if (typeof showMessage === 'function') showMessage('별가루 조회 에러', 'error');
      console.error('[별가루 조회 에러]', e);
      return 0;
    }
  }
  ...
  async setCurrentUserStars(newStars) {
    try {
      ...
      if (typeof showMessage === 'function') showMessage('별가루 저장 에러', 'error');
      console.error('[별가루 저장 에러]', e);
      return 0;
    }
  }
  ...
  async getUserProfile() {
    try {
      ...
      if (typeof showMessage === 'function') showMessage('프로필 조회 에러', 'error');
  ...
  async setUserProfile(profile) {
    try {
      ...
      if (typeof showMessage === 'function') showMessage('프로필 저장 에러', 'error');
  ...
  async syncUserProfile() {
    try {
      const ref = this.getUserRef();
      if (!ref) return { userName: null, stars: 0 };
      const [db, col, uid] = ref;
      ...
      if (typeof showMessage === 'function') showMessage('프로필 동기화 에러', 'error');
      console.error('[프로필 동기화 에러]', e);
      return { userName: null, stars: 0 };
    }
  }
